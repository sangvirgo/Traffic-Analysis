...
model = torch.hub.load('ultralytics/yolov5', 'custom', path='C:\\Traffic-Analysis\\best.pt', force_reload=True)
...
def xuatloi(vid_path):
    ...
    while cap.isOpened():
        ...
        for det in results.xyxy[0]:
            clsid = int(det[-1])
            if clsid in [0, 1, 2, 3]:
                x1, y1, x2, y2 = map(int, det[:4])#in ra cai loi vi pham tren bouding box(mid x vaf tren cung cua y)
                midx = (x1 + x2) // 2
                topy = y1
                ...
                for bienso, kieuvp in viphams:
                    ...
                    break
        cv2.imshow('Bat loi vi pham', frame)
